...
col_drop = df_airbnb.loc[:, ['listing_url', 'scrape_id', 'last_scraped', 'name', 'summary', 'space', 'description',
                             'experiences_offered',
                             'neighborhood_overview', 'notes', 'transit', 'access', 'interaction', 'house_rules',
                             'thumbnail_url',
                             'medium_url', 'picture_url', 'xl_picture_url', 'host_url', 'host_id', 'host_name',
                             ...
                             'is_location_exact', 'weekly_price',
...
print(
    "Description of target: price suggests that there might be outliers - 75% properties have a price that is lower than 110 yet the highest " \
...
print(
    'The box plot distribution and description of the security fee and cleaning fee shows that 75% of all airbnbs has only cleaning upto '
...
df_airbnb.loc[:, 'host_is_superhost'] = df_airbnb.loc[:, 'host_is_superhost'].map({'f': 0, 't': 1}).astype(bool).astype(
    float)
...
df_airbnb.loc[:, 'instant_bookable'] = df_airbnb.loc[:, 'instant_bookable'].map({'f': 0, 't': 1}).astype(bool).astype(
    float)
...
results = Counter()
df_airbnb['amenities'].str.strip('{}').str.replace('"', '').str.lstrip('\"').str.rstrip('\"').str.split(',').apply(
    results.update)
results.most_common()
...
df_airbnb['amenities_Air conditioning'] = df_airbnb['amenities'].str.contains('Air conditioning').astype(bool).astype(
    float)
...
df_airbnb['amenities_Laptop friendly workspace'] = df_airbnb['amenities'].str.contains(
    'Laptop friendly workspace').astype(bool).astype(float)
